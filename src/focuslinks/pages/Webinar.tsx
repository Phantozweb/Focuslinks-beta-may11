                    ...
                    <div className="relative p-6 sm:p-8">
                      ...
                      <div className="flex items-center gap-4 mb-5">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">If you attended the Beyond Ortho-K masterclass, enter your details to claim your certificate of participation.</p>
                        </div>
                      </div>
                      ...
                      {isFeedbackOpen && (
                        <div className="p-5 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div><h3 className="text-base sm:text-lg font-bold">Share Feedback</h3><p className="text-xs text-slate-500">Help us improve</p></div>
                          </div>
                          {feedbackSubmitted ? <div className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 p-4 rounded-xl text-sm font-medium flex items-center gap-2"><CheckCircle2 className="w-5 h-5" /> Submitted!</div> : (
                            <form onSubmit={handleFeedbackSubmit} className="space-y-3">
                              <textarea required rows={4} placeholder="How was the session?" value={feedback} onChange={e => setFeedback(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-all resize-none placeholder:text-slate-400" />
                              <button type="submit" disabled={isSubmitting} className="w-full py-2.5 bg-violet-600 hover:bg-violet-700 disabled:opacity-70 text-white rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                                {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : <><Send className="w-4 h-4" /> Submit</>}
                              </button>
                            </form>
                          )}
